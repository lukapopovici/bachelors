...
MSV-med is a FastAPI service for working with DICOM studies in Orthanc. It includes asynchronous Celery jobs, PostgreSQL metadata and vector search, Prometheus request metrics, a DearPyGui desktop client, and a recursive DICOM importer.
...
The default Compose stack contains a one-shot migration job, one API service,
three queue-specific Celery workers, PostgreSQL, Redis, and Orthanc. The GUI is
an optional host/X11 service and is not required by the backend.
...
- DearPyGui study browsing, ingestion, forwarding, jobs, search, administration, and activity views.
- Recursive DICOM importer with dry-run, deep-scan, confirmation, and parallel upload options.
...
The API requires a bearer token. For the default local configuration, log in to
get one:
...
The health response should contain `"api":"ok"` and `"pacs_reachable":true`.
The metrics endpoint is intended for an internal Prometheus scrape and should
not be exposed directly to the public internet. Domain API routes are available
under the stable `/api/v1` prefix, for example `/api/v1/auth/login` and
`/api/v1/studies`; the original unversioned paths remain available for
backward compatibility. Open these in a browser:

- API documentation: `http://localhost:8000/docs`
- ReDoc documentation: `http://localhost:8000/redoc`
- Orthanc: `http://localhost:8042` (login `orthanc`, password `orthanc` by default)
...
The GUI logs in automatically with `API_USERNAME` and `API_PASSWORD` when
`API_TOKEN` is not set. For a non-default installation, use the credentials
...
The development override mounts `src/` into the API container and enables
Uvicorn reload. *It is not intended for production.*

## 3. AI indexing and search
...
Uploads require valid DICOM files with SOP Instance, Study Instance, and Series
Instance UIDs. When anonymization is enabled, identifying tags and private tags
are removed and UIDs are remapped consistently across the job. The current
anonymization is intended for controlled development and staging workflows;
validate it against your organization’s approved DICOM confidentiality profile
before using it with clinical data in production.
...
PostgreSQL data is stored in the `postgres_data` Docker volume. Orthanc data is
stored in the `orthanc_data` volume. `docker compose down` preserves both;
`docker compose down -v` permanently deletes them. Failed DICOM instances are
stored as binary data for retry, so protect the database volume as PHI.
...
The Docker Compose file includes development defaults, so a fresh clone can start
without creating an environment file:

```bash
docker compose up --build -d
```

For a local test run, the built-in defaults use `admin` / `admin123` and
`changeme` for the legacy API secret. Change these values before exposing the
service to another machine or network. You can either export variables in your
shell or create an untracked `.env` file to override them. Do not commit `.env`.
...
`DATABASE_URL`, `REDIS_URL`, and `ORTHANC_URL` must use Docker service names
inside containers and `localhost` when the process runs directly on the host.
...
On first API startup, an administrator is created automatically if `ADMIN_USERNAME`
does not already exist. Existing users and passwords are never overwritten.
...
The migration job runs once and must complete successfully before the API and
workers start. The stack then runs PostgreSQL, Redis, Orthanc, the API, and
three queue-specific Celery workers:

- `worker`: DICOM upload processing
- `worker-forward`: PACS forwarding
- `worker-demo`: UI/demo jobs used for testing concurrency and job job representation in the database during development. 

PostgreSQL is initialized with pgvector from `docker/initdb/01-enable-pgvector.sql`.
The migration job applies files in `alembic/versions/` and exits after success;
an exited migration container is expected.
...
The Docker defaults are for local development only. They are deliberately
convenient for a fresh clone and are not suitable for an internet-facing
deployment.

## 10. DICOM integration, privacy, and security
